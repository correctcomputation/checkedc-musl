...
1. Pointer arithmetic done to a pointer (buf += sizeof(char *)-i) 

Solution: 
making our parameter be named buf_ori allows all the funky 
pointer arithmetic that follows in the function to happen 
without confusing the compiler, but also enforces the invariant 
that buflen will be precisely the count of buf when the arguments 
are passed into this function. 
	
**************************************************** */

int gethostbyaddr_r(const void *a, socklen_t l, int af,
	struct hostent *h, char *buf/*rename to buf_ori*/, size_t buflen,
