...
1. Incrementing a pointer variable that was passed in as a parameter
	Evidence: s += j+1, *++s 

Root problem: Funky pointer arithmetic threatens the checkedness
of the parameter being passed into the function, so create a temp

Solution: Create temporary variable sw to represent s. 
Author's note: This one isn't EXACTLY like dn_comp.c in that 
dn_comp.c is very "deterministic" whereas if you examine the 
for loops below, you can see that based on some guard, we 
may preemptively exit the for loop, which means the bounds of 
s can be anywhere from 0 to length 4 at its max. 
Yahui reasons that the best we can do given a less than ideal situation
like this is to assume s has length 4 and move on, but it may 
be worth considering if this is the approach we want to take. 
